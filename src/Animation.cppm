...
concept animation_c = requires(T t, duration_t time) {
...
struct eval_animation_impl<TAnimation1, TAnimations...> {
    ...
    static bool
    should_evaluate(duration_t time, TAnimation1& anim1, TAnimations&... anims) noexcept
    {
    ...
    static glm::mat4
    evaluate(glm::mat4 current,
             duration_t time,
             duration_t frame_time,
             TAnimation1& anim1,
             TAnimations&... rest) noexcept
    {
        if (anim1.should_evaluate(time)) {
...
struct eval_animation_impl<> {
    ...
    should_evaluate(duration_t time) noexcept
    {
        return false;
    }

    static constexpr glm::mat4
    evaluate(glm::mat4 current, duration_t time, duration_t frame_time) noexcept
    {
        return current;
    }
};
...
glm::mat4
evaluate_animations(glm::mat4 current, duration_t time, duration_t frame_time, animation_t&... animations)
{
...
export template<animation_c... animation_t>
glm::mat4
evaluate_animations(glm::mat4 current, animation_t&... animations)
{
    static obscure::stopwatch<float> animation_timer{};
    return evaluate_animations<animation_t...>(
        current, animation_timer.total_time(), animation_timer.lap_time(), animations...);
}
...
struct Combine {
    ...
    constexpr Combine(TAnimations... animations)
        : animations(std::make_tuple(animations...))
    {
    }

    template<animation_c... TAnimations2>
    static constexpr Combine<TAnimations..., TAnimations2...>
    combine(Combine a, Combine<TAnimations2...> b) noexcept
    {
        return std::apply(
            [b](TAnimations... anims) {
                return std::apply(
                    [anims...](TAnimations2... anims2) {
    ...
    {
        return std::apply(
            [current, time, frame_time](TAnimations&... anims) {
...
constexpr Combine<TA1, TA2>
operator&(TA1 a, TA2 b) noexcept
{
    return Combine<TA1, TA2>{ std::move(a), std::move(b) };
}

export template<animation_c TA1, animation_c... TAnimations>
constexpr Combine<TA1, TAnimations...>
operator&(TA1 a, Combine<TAnimations...> b) noexcept
{
    return std::apply(
        [a](TAnimations... anims) { return Combine<TA1, TAnimations...>{ std::move(a), std::move(anims)... }; },
        b.animations);
}
...
export struct Move {
    glm::vec3 velocity;
    constexpr bool
    should_evaluate(duration_t time) const noexcept
    {
        return true;
    }

    constexpr glm::mat4
    evaluate(glm::mat4 current, duration_t time, duration_t frame_time) const noexcept
    {
...
export template<animation_c TAnimation>
Duration<TAnimation>
operator&(TAnimation anim, duration_t time) noexcept
{
...
export template<animation_c TAnimation>
Duration<TAnimation>
operator&(duration_t time, TAnimation anim) noexcept
{
    return Duration<TAnimation>{ anim, time };
}
...
struct sequence_impl<TA1, TAnimations...> {
    static bool
    should_evaluate(duration_t time, Duration<TA1>& d1, Duration<TAnimations>&... rest) noexcept
    {
        if (d1.duration >= time) {
    ...
    evaluate(glm::mat4 current,
             ...
             duration_t frame_time,
             Duration<TA1>& d1,
             Duration<TAnimations>&... rest) noexcept
    {
        if (d1.duration >= time) {
    ...
    static duration_t
    total_duration(Duration<TA1> d1, Duration<TAnimations>... rest) noexcept
    {
...
struct sequence_impl<> {
    ...
    static constexpr duration_t
    total_duration() noexcept
    {
        return duration_t{ 0 };
    }
};
...
struct Sequence {
    ...
    {
        return std::apply(
            [time](Duration<TAnimations>&... anims) {
    ...
    {
        return std::apply(
            [current, time, frame_time](Duration<TAnimations>&... anims) {
    ...
    template<animation_c... TAnimations2>
    static constexpr Sequence<TAnimations..., TAnimations2...>
    combine(Sequence const& a, Sequence<TAnimations2...> const& b) noexcept
    {
        return std::apply(
            [&b](Duration<TAnimations> const&... anims) {
                return std::apply(
                    [&anims...](Duration<TAnimations2> const&... anims2) {
...
export template<animation_c TA1, animation_c TA2>
Sequence<TA1, TA2>
operator|(Duration<TA1> a1, Duration<TA2> a2) noexcept
{
...
export template<animation_c TA1, animation_c... TA2>
Sequence<TA1, TA2...>
operator|(Duration<TA1> a1, Sequence<TA2...> a2) noexcept
{
    return std::apply([a1](Duration<TA2>... a2) { return Sequence<TA1, TA2...>{ a1, a2... }; }, a2);
}

export template<animation_c TA1, animation_c... TA2>
Sequence<TA2..., TA1>
operator|(Sequence<TA2...> s, Duration<TA1> a2) noexcept
{
    return std::apply([a2](Duration<TA2>... a1) { return Sequence<TA2..., TA1>{ a1..., a2 }; }, s.animations);
}

export template<animation_c TA1, animation_c... TA2>
    requires(is_sequence<TA1>())
auto
operator|(TA1 a1, Sequence<TA2...> a2) noexcept
{
...
struct Loop {
    ...
    [[nodiscard]] duration_t
    total_duration() const noexcept
    {
