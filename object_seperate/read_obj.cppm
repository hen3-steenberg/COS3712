...
export std::vector<std::pair<rapidobj::Material, std::vector<triangle>>> read_obj_file(std::filesystem::path const& path) {
    ...
    if (!res.error) {
        ...
        for (auto const& shape : res.shapes) {
            for (size_t triangle_idx = 0; triangle_idx < shape.mesh.material_ids.size(); ++triangle_idx) {
                ...
                new_objects[material_idx].second
                    .emplace_back(triangle
                        {
